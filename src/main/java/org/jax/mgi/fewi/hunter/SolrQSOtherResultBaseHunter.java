...
@Repository
public class SolrQSOtherResultBaseHunter extends SolrHunter<QSOtherResult> {

	public SolrQSOtherResultBaseHunter() {
		...
		propertyMap.put(SearchConstants.QS_OBJECT_TYPE, new SolrPropertyMapper(IndexConstants.QS_OBJECT_TYPE));
		propertyMap.put(SearchConstants.QS_OBJECT_SUBTYPE, new SolrPropertyMapper(IndexConstants.QS_OBJECT_SUBTYPE));
	...
	@Override
	protected void packInformation (QueryResponse rsp, SearchResults<QSOtherResult> sr, SearchParams sp) {

		logger.debug ("Entering SolrQSOtherResultBaseHunter.packInformation()");
		...
		for (SolrDocument doc : sdl) {
			...
			try {
				...
				result.setObjectType((String) doc.getFieldValue(IndexConstants.QS_OBJECT_TYPE));
				result.setObjectSubtype((String) doc.getFieldValue(IndexConstants.QS_OBJECT_SUBTYPE));
	 ...
	 */
	private void packFacetData (QueryResponse rsp, SearchResults<QSOtherResult> sr) {
