	...
	public SearchResults<QSOtherResult> getOtherSnpResults(List<String> terms) {
		...
		for (String term : terms) {
			if ((term != null) && (term.startsWith("rs"))) {
				SearchResults<ConsensusSNPDocument> snpResults = snpFinder.getSnpByID(term);

				for (ConsensusSNPDocument snp : snpResults.getResultObjects()) {
					QSOtherResult result = new QSOtherResult();
					result.setStars("****");
					result.setDetailUri("/snp/" + snp.getObjectJSONData().getAccid());
					result.setObjectType(snp.getObjectJSONData().getVariationClass());
					result.setPrimaryID(snp.getObjectJSONData().getAccid());
					result.setSearchTermDisplay(snp.getObjectJSONData().getAccid());
					result.setSearchTermType("SNP ID");
					result.setSearchTermExact(snp.getObjectJSONData().getAccid());
					result.setSearchTermWeight(100);
					result.setSequenceNum(1L);
					result.setUniqueKey(snp.getObjectJSONData().getAccid());
					
					String name = snp.getObjectJSONData().getAccid();
					...
					name = name + ", " + snp.getObjectJSONData().getVariationClass() + ", " + snp.getObjectJSONData().getAlleleSummary();
