...
public class GOGraphConverter
{
    ...
    */
	...
	String pythonWI = ContextLoader.getConfigBean().getProperty("WI_URL");

	// base URL for the Java WI (including 'servlet', above WIFetch)
	String javaWI = ContextLoader.getConfigBean().getProperty("JAVAWI_URL");

	// base URL for the Java WI (including 'servlet', above WIFetch)
	String fewi = ContextLoader.getConfigBean().getProperty("FEWI_URL");

	// base URL for MGI Home (its www directory)
	String mgiHome = ContextLoader.getConfigBean().getProperty(
		"MGIHOME_URL");

	// base URL for GO graphs
        String imageUrl = ContextLoader.getConfigBean().getProperty(
		"GO_GRAPH_URL");
