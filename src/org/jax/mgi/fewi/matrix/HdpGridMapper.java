...
public class HdpGridMapper {
	...
	/*
	 * colIdList is the ordered list of columns by their identifier (could be ID
	 * or term name) gridAnnotations is the data that we have (to be matched by
	...
	private void init() {
		...
		for (HdpGridAnnotation annot : gridAnnotations) {
			...
			if (annotationMap.containsKey(annotId)) {
				if (annot.getAnnotCount() > 0) {
					// aggregate all the annot counts that we come accross for
					// this column in this row
					HdpGridAnnotation prevAnnot = annotationMap.get(annotId);
					Integer newAnnotCount = prevAnnot.getAnnotCount()
							+ annot.getAnnotCount();
				...
				if (annot.getHumanAnnotCount() > 0) {
					...
					Integer newHumanAnnotCount = prevAnnot.getHumanAnnotCount()
							+ annot.getHumanAnnotCount();
		...
		for (String colId : colIdList) {
			...
			if (annotationMap.containsKey(colId))
				annot = annotationMap.get(colId);
			...
			if (annot != null) {
				...
				// (applies to phenotype popup only)
				if (annot instanceof HdpGenoClusterAnnotation
						&& ((HdpGenoClusterAnnotation) annot)
								.getHasBackgroundNote()) {
	...
	/*
	 * Bean that represents a grid cell (correlates to each <td> in the JSP) Has
	 * some attributes to determine which symbols or links to display
	 */
	public class GridCell {
		...
		public int getDiHumanBin() {
			...
			if (humanBin > 0 && annotCount < 1)
				humanBin += 4; // we shift the numbering up to get to the styles
								// for full color squares
			return humanBin;
		}
